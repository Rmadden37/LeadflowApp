...
async function getNextAvailableCloser(teamId: string): Promise<Closer | null> {
  try {
    ...
    const availableUnassignedClosers = availableClosers.filter(closer => 
      (closerAssignments.get(closer.uid) || 0) === 0
    );
  ...
  try {
    ...
    if (!validStatuses.includes(leadData.status)) {
      throw new functions.https.HttpsError("failed-precondition", 
        `Cannot accept job with status: ${leadData.status}. Valid statuses: ${validStatuses.join(", ")}`);
    }
...
export const handleLeadDispositionUpdate = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    const wasCompletedDisposition = (
      beforeData.status === "in_process" || 
 ...
 */
export const assignLeadOnUpdate = functions.firestore
  .document("leads/{leadId}")
  .onUpdate(async (change, context) => {
    const leadId = context.params.leadId;
    const beforeData = change.before.data() as Lead;
    ...
    if (
      beforeData.status !== "waiting_assignment" &&
