...
const responsePatterns = [
    {
        ...
        response: (context) => `☀️ Greetings, devoted ${context.userRole}! By the eternal light of the sun, I am Ra, your divine guide through the LeadFlow realm. How may I illuminate your path today?`
    },
    {
        keywords: ['help', 'assist', 'guide'],
        response: (context) => `The sacred wisdom reveals many paths, mortal one. As a ${context.userRole}, you may seek guidance on:\n\n🌟 Lead history and assignment\n🌟 Best practices for your role\n🌟 System navigation\n🌟 Team coordination\n🌟 Performance optimization\n\nSpeak your specific need, and the solar energy shall guide you!`
    },
    {
        ...
        response: async (context) => {
            ...
            if (csvInsights && csvInsights.topClosers.length > 0) {
                const topClosersList = csvInsights.topClosers
                    .map((closer, index) => `${index + 1}. ${closer.name}: ${closer.totalKW.toFixed(1)} kW (${closer.totalDeals} deals)`)
                    .join('\n');
        ...
        response: async (context) => {
            ...
            if (csvInsights && csvInsights.topClosers.length > 0) {
                const allClosers = csvInsights.topClosers
                    .map((closer, index) => `🌟 ${closer.name}: ${closer.totalKW.toFixed(1)} kW across ${closer.totalDeals} net deals`)
                    .join('\n');
        ...
        response: async (context) => {
            ...
            if (setterInsights && setterInsights.topSetters.length > 0) {
                const topSettersList = setterInsights.topSetters
                    ...
                    .map((setter, index) => `${index + 1}. ${setter.name}: ${setter.conversionRate.toFixed(1)}% conversion (${setter.soldLeads}/${setter.totalLeads} leads)`)
                    .join('\n');
        ...
        response: async (context) => {
            ...
            if (setterInsights && setterInsights.topSetters.length > 0) {
                const allSetters = setterInsights.topSetters
                    ...
                    .map((setter, index) => `🌟 ${setter.name}: ${setter.conversionRate.toFixed(1)}% conversion (${setter.soldLeads} sold from ${setter.totalLeads} leads)`)
                    .join('\n');
    ...
    },
    {
        ...
        response: async (context) => {
            ...
            if (csvInsights && csvInsights.topClosers.length > 0) {
                ...
                csvInsights.topClosers.forEach((closer, index) => {
            ...
            if (setterInsights && setterInsights.topSetters.length > 0) {
                ...
                setterInsights.topSetters.slice(0, 5).forEach((setter, index) => {
    ...
    },
    {
        ...
        response: async (context) => {
            ...
            const maxKW = Math.max(...topClosers.map((c) => c.totalKW));
            ...
            topClosers.forEach((closer, index) => {
        ...
        response: async (context) => {
            ...
            if (csvInsights && csvInsights.topClosers.length > 0) {
                ...
                response += JSON.stringify({
                    closers: csvInsights.topClosers.map((c) => ({
                        name: c.name,
            ...
            if (setterInsights && setterInsights.topSetters.length > 0) {
                ...
                setterInsights.topSetters.slice(0, 5).forEach((setter) => {
    ...
    },
    {
        ...
        response: async (context) => {
            ...
            if (csvInsights && csvInsights.topClosers.length > 0) {
                ...
                const maxCloserKW = Math.max(...topClosers.map((c) => c.totalKW));
                topClosers.forEach((closer, index) => {
            ...
            if (setterInsights && setterInsights.topSetters.length > 0) {
                ...
                const maxSetterConv = Math.max(...topSetters.map((s) => s.conversionRate));
                topSetters.forEach((setter, index) => {
...
async function checkForCloserNameLookup(message, context) {
    const csvInsights = await getCSVInsights(context.userRole, context.teamId);
    if (!csvInsights || csvInsights.topClosers.length === 0) {
        return null;
    }
    ...
    for (const closer of csvInsights.topClosers) {
        ...
        if (nameParts.some((part) => part.length > 2 && messageLower.includes(part))) {
            const rank = csvInsights.topClosers.findIndex((c) => c.name === closer.name) + 1;
...
async function fetchCloserDataFromCSV() {
    var _a, _b, _c, _d;
    try {
        ...
        for (let i = 1; i < Math.min(lines.length, 500); i++) {
            try {
                ...
                if (row.length < 3)
                    continue;
                const closer = closerIndex >= 0 ? (_b = (_a = row[closerIndex]) === null || _a === void 0 ? void 0 : _a.replace(/['"]/g, '')) === null || _b === void 0 ? void 0 : _b.trim() : '';
                const systemSize = systemSizeIndex >= 0 ? parseFloat((_c = row[systemSizeIndex]) === null || _c === void 0 ? void 0 : _c.replace(/['"]/g, '')) || 0 : 0;
                const realization = realizationIndex >= 0 ? parseFloat((_d = row[realizationIndex]) === null || _d === void 0 ? void 0 : _d.replace(/['"]/g, '')) || 0 : 0;
...
async function getCSVInsights(userRole, teamId) {
    try {
...
async function getSetterInsightsFromCSV(userRole, teamId) {
    ...
    try {
        ...
        for (let i = 1; i < Math.min(lines.length, 500); i++) {
            try {
                ...
                if (row.length < 3)
                    continue;
                const setter = setterIndex >= 0 ? (_b = (_a = row[setterIndex]) === null || _a === void 0 ? void 0 : _a.replace(/['"]/g, '')) === null || _b === void 0 ? void 0 : _b.trim() : '';
                const closer = closerIndex >= 0 ? (_d = (_c = row[closerIndex]) === null || _c === void 0 ? void 0 : _c.replace(/['"]/g, '')) === null || _d === void 0 ? void 0 : _d.trim() : '';
                const systemSize = systemSizeIndex >= 0 ? parseFloat((_e = row[systemSizeIndex]) === null || _e === void 0 ? void 0 : _e.replace(/['"]/g, '')) || 0 : 0;
                const realization = realizationIndex >= 0 ? parseFloat((_f = row[realizationIndex]) === null || _f === void 0 ? void 0 : _f.replace(/['"]/g, '')) || 0 : 0;
...
async function checkForSetterNameLookup(message, context) {
    const setterInsights = await getSetterInsightsFromCSV(context.userRole, context.teamId);
    if (!setterInsights || setterInsights.topSetters.length === 0) {
        return null;
    }
    ...
    for (const setter of setterInsights.topSetters) {
        ...
        if (nameParts.some((part) => part.length > 2 && messageLower.includes(part))) {
            const rank = setterInsights.topSetters.findIndex((s) => s.name === setter.name) + 1;
...
async function processLeadflowAssistant(input) {
    ...
    for (const pattern of responsePatterns) {
        if ('keywords' in pattern && pattern.keywords.some((keyword) => messageText.includes(keyword))) {
