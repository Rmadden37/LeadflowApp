  ...
  try {
    ...
    if (analyticsQ.includes('top') && analyticsQ.includes('closer')) {
      ...
      if (closersArr.length > 0) {
        const top = closersArr.slice(0, 5).map((c: any, i: number) => `${i + 1}. ${c.name} - ${c.sales} sales, $${c.revenue?.toLocaleString?.() || c.revenue} revenue, ${c.totalKW} kW`).join('\n');
    ...
    if (analyticsQ.includes('top') && analyticsQ.includes('setter')) {
      ...
      if (settersArr.length > 0) {
        const top = settersArr.slice(0, 5).map((s: any, i: number) => `${i + 1}. ${s.displayName} - ${s.totalLeads} leads`).join('\n');
    ...
    if (analyticsQ.includes('conversion rate')) {
      ...
      if (closersArr.length > 0) {
        const avg = (closersArr.reduce((sum: number, c: any) => sum + (c.sales || 0), 0) / (closersArr.length || 1)).toFixed(2);
