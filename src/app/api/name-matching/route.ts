...
export async function GET() {
  try {
    ...
    function findUserProfile(nameOrEmail: string, users: any[]): any | undefined {
      ...
      const nameParts = trimmedName.split(' ').filter((p: string) => p.length > 0)
      if (nameParts.length >= 2) {
        ...
        matched = users.find(u => {
          ...
          const userParts = u.displayName.trim().split(' ').filter((p: string) => p.length > 0)
    ...
    return NextResponse.json({ error: 'Failed to process name matching', details: message }, { status: 500 });
  }
}
