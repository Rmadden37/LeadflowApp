...
export function useSwipeNavigation(config: SwipeNavigationConfig = {}) {
  ...
  // OPTIMIZED: Use requestAnimationFrame for 60fps smooth updates (Reanimated pattern)
  const handleTouchMove = useCallback((e: TouchEvent) => {
    ...
    if (deltaX > 0) {
      ...
      // CRITICAL OPTIMIZATION: Use requestAnimationFrame to batch state updates for smooth 60fps
  ...
  // OPTIMIZED: Hardware-accelerated completion animation
  const completeNavigation = useCallback(() => {
    setSwipeState(prev => ({ ...prev, progress: 1 }));
    
    // Smooth animation to completion using requestAnimationFrame
    const animate = () => {
      setSwipeState(prev => {
        if (prev.progress >= 1) {
  ...
  // OPTIMIZED: Smooth cancellation animation
  const cancelNavigation = useCallback(() => {
    // Smooth animation back to 0 using requestAnimationFrame
    const animate = () => {
      setSwipeState(prev => {
        if (prev.progress <= 0) {
          return { ...prev, isActive: false, progress: 0 };
