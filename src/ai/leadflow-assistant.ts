  ...
  async (input) => {
    ...
    if (context.analytics) {
      ...
      if (closers.length > 0) {
        ...
        analyticsSummary += '\n' + (closers as Closer[]).slice(0, 5).map((c: Closer, i: number) => `${i + 1}. ${c.name} | ${c.sales} | $${c.revenue?.toLocaleString?.() || c.revenue} | ${c.totalKW}`).join('\n');
      }
      interface Setter {
        displayName: string;
        totalLeads: number;
      ...
      if (setters.length > 0) {
        ...
        analyticsSummary += '\n' + (setters as Setter[]).slice(0, 5).map((s: Setter, i: number) => `${i + 1}. ${s.displayName} | ${s.totalLeads}`).join('\n');
      }
